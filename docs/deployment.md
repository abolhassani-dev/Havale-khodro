...
هر خطای پیش‌بینی‌نشده‌ی سامانه در جدول `ErrorLog` ثبت می‌شود — با متن کامل، محل وقوع،
تعداد تکرار، و **یک پیشنهاد مشخص برای شروع دیباگ**. خطاهای یکسان روی یک ردیف جمع
می‌شوند تا یک باگ پرتکرار بقیه را پنهان نکند.

اگر بات تلگرام تنظیم شود، همان خطا **همان لحظه** به گوشی شما هم می‌رسد، با همان راهنما.

> **در پنل مدیریت صفحه‌ای برای این ندارد، و عمداً.** لاگ خطا کار فنی است، نه عملیاتی؛
> نشان دادن stack trace به کسی که دارد درخواست ظرفیت تأیید می‌کند به هیچ‌کس کمکی
> نمی‌کند. API‌اش آماده و تست‌شده است (`GET /api/v1/errors`) تا «پنل اونر» آینده یک
> صفحه باشد نه یک پروژه. ثبت خطا و هشدار تلگرام مستقل از هر پنلی کار می‌کنند.

**تا آن روز، دیدنشان روی سرور:**

```bash
cd /opt/feranocar
docker compose exec -T db psql -U havale havale -c \
  'SELECT message, count, "lastSeen", path FROM "ErrorLog" WHERE "resolvedAt" IS NULL ORDER BY "lastSeen" DESC LIMIT 20;'
```

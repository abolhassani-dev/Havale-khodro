...
با این دو کلید، هر بار که سیستم بالا می‌آید چهار نمایندگی نمونه، چند حواله و ده
آگهی خودروی نمونه ساخته می‌شوند (تکراری نمی‌سازد). اگر سیستم بالاست و فقط
آگهی‌های خودرو را می‌خواهید، **داخل کانتینر** اجرا کنید — روی خود سرور `npm` نیست:

```bash
docker compose exec api node scripts/seed-cars.js
```
...
**ابر نارنجی نه.** دلیلش را در `archive/docs/staging.md` بخش ۷ نوشته‌ام: کلادفلیر داخل ایران PoP ندارد،
...
## هشدار به گوشی (ربات بله) و لاگ خطاها
...
می‌شوند تا یک باگ پرتکرار بقیه را پنهان نکند. مالک آن‌ها را در پنل خودش (لاگ فنی)
می‌بیند.

اگر ربات هشدار تنظیم شود، همان خطا **همان لحظه** به گوشی شما هم می‌رسد، با همان
راهنما — و دیده‌بان (بخش بعد) هم از همین کانال استفاده می‌کند.

> **چرا بله و نه تلگرام؟** سرور در ایران است و تلگرام از داخل ایران در دسترس
> نیست. پیام‌رسان **بله** دقیقاً همان Bot API تلگرام را دارد، پس کل کد بدون تغییر
> با آن کار می‌کند؛ فقط آدرس فرق دارد (`ALERT_API_BASE=https://tapi.bale.ai`،
> که پیش‌فرض است). اگر روزی از سروری خارج از ایران استفاده کردید، همان متغیر را
> به `https://api.telegram.org` تغییر دهید و ربات را در تلگرام بسازید.

### راه‌اندازی (پنج دقیقه، یک بار)

**۱.** در برنامه‌ی **بله** روی گوشی، `@BotFather` را پیدا کنید و `/newbot` بزنید —
یک نام و یک username بدهید. توکن را می‌دهد. **توکن را در هیچ چتی نفرستید.**

**۲.** به ربات تازه‌ساخته‌تان در بله یک پیام بفرستید (هر چیزی، مثلاً `سلام`). بدون
این، ربات اجازه ندارد به شما پیام بدهد.

**۳.** روی سرور:

```bash
cd /opt/feranocar && ./deploy/alert-setup.sh
```

اسکریپت اول می‌بیند بله از سرور در دسترس است، بعد توکن را **از خودتان** می‌پرسد
(تایپ می‌کنید، چیزی چاپ نمی‌شود)، chat id را خودش پیدا می‌کند، هر دو را در `.env`
می‌گذارد، API را بالا می‌آورد و یک پیام آزمایشی می‌فرستد. اگر پیام روی گوشی آمد،
تمام است.

**۴. بررسی بعدی، هر وقت خواستید:** پنل مالک → لاگ فنی → دکمه‌ی **«ارسال پیام
آزمایشی»**؛ یا روی سرور `./deploy/alert-probe.sh`.

> کانالی که کسی امتحانش نکرده، کانال هشدار نیست. این دکمه دقیقاً برای همین است.

> اگر ربات از سرور در دسترس نباشد، هشدار ارسال نمی‌شود و **هیچ اتفاقی برای برنامه
...
رفعش** به ربات هشدار (بله) می‌فرستد:
...
### اگر build با خطای شبکه (`ETIMEDOUT`) متوقف شد

**اول از همه: سایت پایین نیامده.** build شکست‌خورده هیچ‌چیزی را جایگزین نمی‌کند و
کانتینرها با نسخه‌ی قبلی سر جایشان می‌مانند.

این خطا فقط وقتی پیش می‌آید که `backend/package.json` عوض شده باشد — تا وقتی
وابستگی‌ها دست‌نخورده باشند، داکر آن مرحله را از کش برمی‌دارد و اصلاً سراغ اینترنت
نمی‌رود. یعنی هر بار که یک پکیج جدید اضافه می‌شود، سرور باید یک بار کل درخت پکیج‌ها را
از npm بگیرد و مسیر این سرور به npm کند است.

**درمان: همان دستور را دوباره اجرا کنید.** کش npm بین بیلدها روی سرور می‌ماند، پس هر
اجرا از جایی که قبلی رسیده ادامه می‌دهد. اگر شبکه خیلی ناپایدار بود، بگذارید خودش تکرار
کند تا موفق شود:

```bash
cd /opt/feranocar
until ./deploy/update.sh; do echo "تلاش دوباره تا دو دقیقه دیگر…"; sleep 120; done
```

**اگر ساعت‌ها ادامه داشت**، یک بار از میرور استفاده کنید — یک خط در `.env`:
...
این کار امن است چون `package-lock.json` برای هر پکیج یک **هش یکپارچگی** دارد و npm هر
فایل دریافتی را با آن می‌سنجد؛ میروری که چیز دیگری بدهد، نصب را متوقف می‌کند. بعد از
موفقیت می‌توانید همان خط را از `.env` بردارید تا بیلدهای بعدی دوباره از رجیستری رسمی
بگیرند.
...
## بررسی کلی — آیا همه‌ی این سند انجام شده؟

هر گام این سند کاری است که کسی باید یادش بماند انجام دهد، و مهم‌ترین‌هایشان دقیقاً
همان‌هایی هستند که بی‌صدا شکست می‌خورند: کرون بکاپی که نصب نشد، بکاپی که هیچ‌وقت از
سرور بیرون نرفت، حساب نمایشی‌ای که فعال ماند، کلید رمزنگاری‌ای که خالی ماند و شماره‌ها
را متن ساده گذاشت. هیچ‌کدام صفحه‌ای را خراب نمی‌کنند — روزی می‌فهمید که لازمشان داشتید.

```bash
/opt/feranocar/deploy/preflight.sh
```

حدود صد مورد را بند به بند بررسی می‌کند و هرجا انجام نشده، **دستور دقیق انجامش** را
کنارش می‌نویسد. چیزی را عوض نمی‌کند: نه ری‌استارتی، نه فایلی، نه هشداری — روی سیستم زنده
هر وقت خواستید بزنید. هیچ رمزی هم چاپ نمی‌کند؛ فقط می‌گوید هست و به‌اندازه‌ی کافی بلند
هست.

خروجی سه حالت دارد: `✓` انجام شده، `!` هشدار (کار می‌کند ولی بهتر است درست شود)،
`✗` انجام نشده. اگر هیچ `✗` نبود، کد خروجی صفر است — پس در کرون هم قابل استفاده است.

```bash
deploy/preflight.sh --full             # به‌علاوه‌ی بررسی‌های کند (تمدید گواهی، مقصد بکاپ، DNS کانتینر)
deploy/preflight.sh --list             # فهرست بخش‌ها
...
`--quiet` وقتی همه‌چیز درست است **هیچ چیزی چاپ نمی‌کند**، پس این کرون فقط روزی که
خبری هست ایمیل می‌فرستد:

```bash
cat > /etc/cron.d/feranocar-preflight <<'CRON'
17 6 * * 1 root /opt/feranocar/deploy/preflight.sh --quiet
CRON
chmod 644 /etc/cron.d/feranocar-preflight
```

### ساختار — برای پروژه‌های بعدی

`preflight.sh` فقط اجراکننده است؛ خودِ بررسی‌ها در `deploy/preflight.d/` هستند، هر
بخش یک فایل، به ترتیب نام. اضافه کردن یک بررسی یعنی یک تابع به یکی از آن فایل‌ها —
هیچ‌وقت دست زدن به اجراکننده.

بخش‌های ۱۰ تا ۲۹ و ۷۰ تا ۹۹ درباره‌ی «یک سرور داکری» می‌پرسند و چیزی از این محصول
نمی‌دانند؛ بین آن‌ها مال این پروژه است. پروژه‌ی بعدی دسته‌ی اول را عیناً برمی‌دارد،
دسته‌ی دوم را عوض می‌کند، و تنظیمات بالای `preflight.sh` را.

راهنمای کامل: [`deploy/preflight.d/README.md`](../deploy/preflight.d/README.md)

> **چرا این جدا از دیده‌بان است؟** دیده‌بان (`watchdog.sh`) هر پنج دقیقه می‌پرسد «الان
> چیزی خراب است؟» و هشدار می‌دهد. این یکی می‌پرسد «آیا از اول درست راه‌اندازی شده؟» —
> سؤالی که فقط بعد از نصب، بعد از هر تغییر سرور، و هر چند وقت یک‌بار ارزش پرسیدن دارد.
...
| کار | دستور |
|---|---|
| **بررسی اینکه همه‌ی مراحل این سند انجام شده** | `/opt/feranocar/deploy/preflight.sh` |

...
server {
    ...
    location /api/ {
        ...
        # Without this the upstream's \`keepalive 32\` never applies: nginx
        # passes the client's Connection header through and opens a new TCP
        # connection to Node for every single API call.

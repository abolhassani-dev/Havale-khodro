...
maybe('car prices', () => {
  ...
  /**
   * Linking the catalogue to the price list, and reading a price back off it.
   *
   * The interesting cases are the refusals: this is the part that could put a
   * number belonging to a different car next to somebody's advertisement.
   */
  describe('what a car advertisement may say the market price is', () => {
    let brand;
    const touched = [];

    // The catalogue is seeded by other suites, so these names may already be
    // there. Reuse rather than insert a duplicate, and put the link back the
    // way it was afterwards so the next suite sees an untouched catalogue.
    const model = async (name) => {
      const found = await prisma.carModel.findFirst({ where: { brandId: brand.id, name } });
      const row = found || (await prisma.carModel.create({ data: { brandId: brand.id, name } }));
      touched.push({ id: row.id, mine: !found });
      return row;
    };

    beforeAll(async () => {
      await carPriceService.refresh('DOMESTIC', { html: domestic });
      // The catalogue is seeded by other suites, so take the real پژو if it is
      // already there — the matcher's maker guard checks this name.
      // ب ام و rather than پژو: the seeded catalogue holds thirteen ۲۰۷s, so
      // «پژو 207 اتوماتیک» is claimed by too many of them and the vagueness
      // rule drops it — which is the rule working, not a fixture to fight.
      brand =
        (await prisma.carBrand.findFirst({ where: { name: 'ب ام و' } })) ||
        (await prisma.carBrand.create({ data: { name: 'ب ام و', slug: `bmw-${Date.now()}` } }));
    });

    afterEach(async () => {
      const rows = touched.splice(0);
      const mine = rows.filter((r) => r.mine).map((r) => r.id);
      const borrowed = rows.filter((r) => !r.mine).map((r) => r.id);
      if (mine.length) await prisma.carModel.deleteMany({ where: { id: { in: mine } } });
      if (borrowed.length) {
        await prisma.carModel.updateMany({
          where: { id: { in: borrowed } },
          data: { priceKey: null, priceKeyLocked: false },
        });
      }
    });

    it('links a car the list covers, and reads its price back', async () => {
      const m = await model('ب ام و سری 7 735i');
      await carPriceService.relink();

      const back = await prisma.carModel.findUnique({ where: { id: m.id }, select: { priceKey: true } });
      expect(back.priceKey).toBeTruthy();

      const market = await carPriceService.marketFor([m.id]);
      const found = market.get(m.id);
      expect(found).toBeTruthy();
      expect(found.min).toBeGreaterThan(0);
      expect(found.max).toBeGreaterThanOrEqual(found.min);
      // The line's own name travels with the number, so a reader can see what
      // the figure is for rather than assuming it is their exact trim.
      expect(found.name).toContain('735');
    });

    it('says nothing for a car the list does not cover', async () => {
      const m = await model('ب ام و ایزتا 300');
      await carPriceService.relink();
      const market = await carPriceService.marketFor([m.id]);
      expect(market.get(m.id)).toBeUndefined();
    });

    it('drops a line that too many of our models claim', async () => {
      // Four cars answering to one price line means the line describes none of
      // them, so none of them gets a price.
      const real = await model('ب ام و سری 7 735i');
      await carPriceService.relink();
      expect((await carPriceService.marketFor([real.id])).size).toBe(1);

      const many = [real];
      for (const suffix of ['لاکچری', 'اسپرت', 'پرمیوم']) {
        // eslint-disable-next-line no-await-in-loop
        many.push(await model(`ب ام و سری 7 735i ${suffix}`));
      }
      await carPriceService.relink();
      // Four cars answering to one line means the line describes none of them,
      // so all four lose the price — including the one that had it.
      expect((await carPriceService.marketFor(many.map((m) => m.id))).size).toBe(0);
    });

    it('leaves a link an admin pinned alone', async () => {
      const m = await model('ب ام و ایزتا 300');
      await prisma.carModel.update({
        where: { id: m.id },
        data: { priceKey: 'دست ساز', priceKeyLocked: true },
      });
      await carPriceService.relink();
      const back = await prisma.carModel.findUnique({ where: { id: m.id }, select: { priceKey: true } });
      expect(back.priceKey).toBe('دست ساز');
    });

    it('shows no price at all while our own snapshot is behind', async () => {
      const m = await model('ب ام و سری 7 735i');
      await carPriceService.relink();
      expect((await carPriceService.marketFor([m.id])).size).toBe(1);

      // A price carrying an old date beside an advertisement is worse than no
      // price, so staleness silences every model rather than some of them.
      const old = new Date(Date.now() - 48 * 60 * 60 * 1000);
      await prisma.carPriceRun.updateMany({ where: { ok: true }, data: { finishedAt: old } });
      expect((await carPriceService.marketFor([m.id])).size).toBe(0);
      await carPriceService.refresh('DOMESTIC', { html: domestic });
      await carPriceService.refresh('IMPORTED', { html: imported });
    });
  });
